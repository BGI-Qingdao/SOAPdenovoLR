...
*************************************************/
#ifdef MER127
void buildReverseComplementEdge ( unsigned int edgeno )
{
...
void loadEdge ( char * graphfile )
{
	...
	long long counter = 0;
        long long cvgSum = 0;
        long long cvg1EdgeCounter = 0;  //number of edge with coverage larger than 1
        long long cvgSumWithoutCvg1Edge = 0;    //coverage sum of edge with coverage larger than 1

	sprintf ( name, "%s.edge.gz", graphfile );
	fp = gzopen ( name, "r" );
	num_ed_limit = 2 * num_ed;
	...
	while ( gzgets ( fp, line, sizeof ( line ) ) != NULL )
	{
		if ( line[0] == '>' )
		{
			if ( index >= 0 )
			{
				...
				edge_array[edgeno].in_path = 0;

				if (length > overlaplen + 1)
                                {
                                        counter += length;
                                        cvgSum += length*cvg;
                                        if (cvg > 10)
                                        {
                                                cvg1EdgeCounter += length;
                                                cvgSumWithoutCvg1Edge += length*cvg;
                                        }
                                }
	...
	if (counter > 0)
        {
                cvgAvg4Edge = cvgSum/counter;
        }

	if (cvg1EdgeCounter > 0)
        {
                cvgAvg4NoneCvg1Edge = cvgSumWithoutCvg1Edge/cvg1EdgeCounter;
        }

	fprintf (stderr,"%d edge(s) input, average coverage is %d, average coverage of edge with coverage larger than 1 is %d.\n", index + 1, cvgAvg4Edge/10, cvgAvg4NoneCvg1Edge/10);
	gzclose ( fp );
	createArcMemo ();
...
void loadPreArcs ( char * graphfile )
{
	...
	while ( fgets ( line, sizeof ( line ), fp ) != NULL )
	{
		...
		while ( ( seg = strtok ( NULL, " " ) ) != NULL )
		{
			target = atoi ( seg );
			seg = strtok ( NULL, " " );
			if(edge_array[from_ed].to_vt == edge_array[target].from_vt)
			{
				weight = atoi ( seg );
