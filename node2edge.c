...
/*
			if ( is_last_kmer == 1 )
			{
				del_node->r_links = 2; // The last kmer of edge in smaller kmer form.
				is_last_kmer = 0;
			}
			else if ( nStack->item_c == 1 )
			{
				del_node->r_links = 1; // The first kmer of edge in smaller kmer form.
			}
			else
			{
				del_node->r_links = 0;
...
/*
			if ( is_last_kmer == 1 )
			{
				del_node->r_links = 1; // The last kmer of edge in larger kmer form.
				is_last_kmer = 0;
			}
			else if ( nStack->item_c == 1 )
			{
				del_node->r_links = 2; // The first kmer of edge in larger kmer form.
			}
			else
			{
				del_node->r_links = 0;
...
*/
	...
	if ( length > 1 )
	{
		...
		if (length == 2)
		{
			second_last_np->node->r_links = 3; // The only linear kmer which serves as the first and the last kmer at the same time.
		}
		*/
	}
	else
