...
static unsigned int * maxfwriteBuf;
...
static void parse1LongRead (int t, int threadID);
			...
			for ( t = 0; t < read_c; t++ )
			{
				...
				for ( j = start; j < finish; j++ )
				{
					if ( flagArray[j] == 0 )
					{
						...
						{
							if ( !repsTie || (((j + 1) < finish) && mixBuffer[j+1].low2 == 0))
							{
								break;
							}
							else
				...
				for ( j = start; j < finish; j++ )
				{
					if ( flagArray[j] == 0 )
					{
						...
						{
							if ( !repsTie || (((j + 1) < finish) && mixBuffer[j+1].low == 0))
							{
								break;
							}
							else
		...
		else if ( * ( prm->selfSignal ) == 6 )
		{
			for ( t = 0; t < read_c; t++ )
			{
				...
				for ( j = start; j < finish - 1; j++ )
				{
					...
					{
						if( !repsTie || (mixBuffer[j].low2 == 0 && mixBuffer[j + 1].low2 == 0))
						{
							break;
						}
						else
				...
				for ( j = start; j < finish - 1; j++ )
				{
					...
					{
						if( !repsTie || (mixBuffer[j].low == 0 && mixBuffer[j + 1].low == 0))
						{
							break;
						}
						else
...
static long long recordLongPathBin (FILE * outfp)
{
	int t, i, j,k, start, finish;
	int counter;
	long long path_num = 0;
	unsigned int path_num_in1read;
	...
	for (t = 0; t < read_c; t++)
	{
...
#ifdef MER127
		if (finish - start < 3 || mixBuffer[start].low2 == 0 || mixBuffer[start + 1].low2 == 0)
		{ //less than 3 edges in read path
			continue;
		}
		...
		for (j = start; j < finish; j++)
		{
			if (mixBuffer[j].low2 == 0)
			{
				...
				{
					path_num++;
					fwrite (&counter, sizeof (int), 1, outfp);
					fwrite (fwriteBuf, sizeof (unsigned int), (int) counter, outfp);
				/*	path_num_in1read++;
				...
				if ((j + 1) < finish && mixBuffer[j+1].low2 == 0)
				{
					break;
				}
			}
...
/*			path_num_in1read++;
	...
	/*	if(path_num_in1read)
                {
                        fwrite (&maxPathLen, sizeof (int), 1, outfp);
                        fwrite (maxfwriteBuf, sizeof (unsigned int), (int) maxPathLen, outfp);
                      	for(k=0;k<maxPathLen;k++)
                        {
                                if(markerOnEdge[maxfwriteBuf[k]] < 255)
                                {
                                        markerOnEdge[maxfwriteBuf[k]]++;
                                }
                                markCounter++;
                        }
                        path_num++;
...
#else
		if (finish - start < 3 || mixBuffer[start].low == 0 || mixBuffer[start + 1].low == 0)
		{
			continue;
		}
		...
		for (j = start; j < finish; j++)
		{
			if (mixBuffer[j].low == 0)
			{
				...
				if ((j + 1) < finish && mixBuffer[j+1].low == 0)
				{
					break;
				}
			}
...
static void search1kmerPlus ( int j, unsigned char thrdID )
{
	...
	if ( smallerBuffer[j] )
	{
		mixBuffer[j].low2 = get_kmer_contig_id(*node); //->l_links;
	}
	else
	{
...
static void parse1read ( int t, int threadID )
{
	...
	for ( j = start; j < finish; j++ )
	{
		...
		if ( node->linear )
		{
			if ( isSmaller )
			{
				edge_index = get_kmer_edge_id(*node);
			}
			else
			{
				edge_index = get_kmer_edge_id(*node) + node->twin - 1;
			}
...
static void parse1LongRead (int t, int threadID)
{
	unsigned int j, retain = 0, maxRetain = 0;
	unsigned int edge_index = 0;
	unsigned int posInEdge;
	kmer_t *node;
	boolean isSmaller;
	Kmer wordplus, bal_wordplus;
	unsigned int start, finish, pos, prePos;
//	int prevEnd = 0, currentEnd;
	Kmer prevKmer, currentKmer;
	boolean IsPrevKmer = 0;
	boolean gotQualified = 0; // whether "retain >= 2" appeared
	boolean isPreQualified = 0;
	...
	// pos always points to the position to be filled
	// when prePos != pos, prePos points to the last filled position
	for (j = start; j < finish; j++)
	{
		node = nodeBuffer[j];

		//extract edges or keep kmers
		if ((node == NULL) || (node->deleted) || (node->linear && !node->inEdge))       // deleted or in a floating loop
		{
			if (retain < 2)
			{
				if (gotQualified == 0)
...
#ifdef MER127
//			if ( retain == 0 || ( IsPrevKmer && ( currentEnd == 1 || currentEnd == 3 )))
			if ( retain == 0 || IsPrevKmer )
			{
				retain++;
				if (maxRetain < retain)
...
//			else if (edge_index != mixBuffer[pos - 1].low2 && ((prevEnd == 2 || prevEnd == 3) && ( currentEnd == 1 || currentEnd == 3 )))
			else if (edge_index != mixBuffer[pos - 1].low2 )
			{
				retain++;
				if (maxRetain < retain)
...
//				prevEnd = currentEnd;
			...
			else
			{
				...
				else if (pos != prePos) // single edge had been added
				{
					mixBuffer[prePos].low2 = edge_index;
					flagArray[prePos] = 0;
					pos = prePos;
				}
...
//			else if (edge_index != mixBuffer[pos - 1].low && ((prevEnd == 2 || prevEnd == 3) && (currentEnd == 1 || currentEnd == 3)))
			else if (edge_index != mixBuffer[pos - 1].low)
			{
				retain++;
				if (maxRetain < retain)
...
//				prevEnd = currentEnd;
			...
			else
			{
				...
				{
					mixBuffer[prePos].low = edge_index;
					flagArray[prePos] = 0;
					pos = prePos;
				}
...
void prlRead2edge ( char * libfile, char * outfile )
{
	...
	if ( repsTie )
	{
		...
		maxfwriteBuf = ( unsigned int * ) ckalloc (( maxReadLen - overlaplen + 1 ) * sizeof ( unsigned int ));
	}
			...
			while ( ( flag = read1seqInLibBam ( seqBuffer[read_c], next_name, & ( lenBuffer[read_c] ), &libNo, pairs, 1, &type ) ) != 0 )
			{
				...
				if ( read_c == maxReadNum )
				{
					...
					*/
					if ( repsTie ) 
					{
						pathCount += recordLongPathBin ( outfp );
					}
			...
			if ( flag1 && flag2 )
			{
				...
				while ( start1 < offset1 || start2 < offset2 )
				{
					if ( turn == 1 )
					{
                        ...
                        {
                            fprintf(stderr,"readseqInLib return error ! please make sure your input file is correct fastq/fasta file.\n");
