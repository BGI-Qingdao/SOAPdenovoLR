...
typedef struct edge
{
	...
	unsigned short cvg: 12; //coverage
	unsigned short bal_edge: 2; // 2:smaller 0:larger 1:rev-com equal to itself
	unsigned short in_path: 1;
	unsigned short inbubble: 1;
	unsigned short multi: 11;
	unsigned short deleted : 1;
	unsigned short flag : 1;
	unsigned short type: 3; // //0: original state; 1: heterozygous; 2: unique; 3: repeat; 4: repeat in path
...
typedef struct pathAcrossEdge
{
	long long int edge_num;
	struct preARC *first_edge;
	struct preARC *last_edge;
